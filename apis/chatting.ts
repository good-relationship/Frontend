...
export const subscribeToMessageTopic = (client: Client, headers: { Authorization: string; }, workspaceId:string, onMessageReceivedCallback: (arg0:GetMessageContentDTO) => void) => {
    client?.subscribe(`/topic/message/${workspaceId}`, (message) => {
        const messageObj = JSON.parse(message.body).body;
        onMessageReceivedCallback(messageObj);
    }, headers);
};

export const subscribeToHistoryTopic = (client: Client, headers: { Authorization: string; }, onHistoryReceivedCallback: (arg0: GetMessageHistoryDTO, arg1: GetMessageContentDTO[]) => void) => {
    client?.subscribe(`/user/queue/history`, (history) => {
